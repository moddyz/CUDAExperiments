...
public:
    ...
    {
        ASSERT( _CopyInitializerList( i_initializerList ) );
    }
    ...
    ~Array()
    {
        if ( m_buffer != nullptr )
        {
            ASSERT( MemoryDeallocate<ResidencyT>::Execute( m_buffer ) );
        }
    }
    ...
    {
        std::stringstream ss;
        ss << "Array< " << DemangledTypeName<ValueT>() << ", " << ResidencyT << " >";
        ss << "[size=" << GetSize() << "]\n";
        return ss.str();
    }
...
private:
    ...
    inline bool _CopyInitializerList( std::initializer_list<ValueT> i_initializerList )
    {
        if ( ResidencyT == CUDA )
        {
            ...
            ASSERT( hostArray.GetBuffer() != nullptr );
